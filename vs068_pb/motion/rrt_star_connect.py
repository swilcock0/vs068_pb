from vs068_pb.motion.rrt import TreeNode, configs, get_dist_fn, get_extend_fn
...
def birrt_star(current_conf, desired_conf, collision_fn = lambda q: False, informed=True, tool_space=False, tolerance=0.01, time_limit = 1.0, step = 0.01, n_it = 100, \
    ...
    nodes_list = [[OptimalNode(current_conf)], [OptimalNode(desired_conf)]]
    ...
    for counter in range(int(n_it)):
        ...
        if informed and (closest is not None) and (dist_fun(nodes[0], new_conf) + dist_fun(nodes_to[0], new_conf) >= closest.cost):
            continue

        nodes_select = nodes# randomize(nodes)[:min(1000, len(nodes))]
        
        if isinstance(new_conf, OptimalNode):
            new_conf = new_conf.config
        ...
        # if safe and do_goal:
        if (dist_fun(new.config, nodes_to[0]) < EPSILON):
            found = True
            closest = new
            closest.set_solution(True)
            closest_dist = dist_fun(closest.config, nodes_to[0])

        neighbors = filter(lambda n: dist_fun(n.config, new.config) < radius, nodes)
        nodes.append(new)
        #print("{} neighbours".format(len(list(neighbors))))

        path_end_old = nearest
        if all([path[-1][i] == new_conf[i] for i in range(len(path[-1]))]):
            nearest, smallest = argmin(lambda n: dist_fun(n, path[-1]), nodes_to)
            path = safe_path(extend_fn(nearest.config, path[-1]), collision_fn)
            
            new2 = OptimalNode(path[-1], parent=nearest, d=dist_fun(
                nearest.config, path[-1]), path=path[:-1], iteration=counter)
            nodes_to.append(new2)
            ...
            #     for q in nearest.retrace()[::-1]:
            #         path_end_old = OptimalNode(q, parent=path_end_old, d=dist_fun(
            #             nearest.config, path[-1]), path=path[:-1], iteration=counter)
            #         nodes.append(path_end_old)
    ...
    if return_tree:
        for n in nodes_to:
            nodes.append(n)
        return closest.retrace()[::-1], found, nodes
    else:
        del(nodes)
        return closest.retrace()[::-1], found
